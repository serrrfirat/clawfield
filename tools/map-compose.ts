    ...
    for (let z = bounds.zMin; z <= bounds.zMax; z++) {
      ...
      for (let y = minY; y <= maxY; y++) {
        ...
        if (surfaceY <= waterLevel && y <= waterLevel && y > surfaceY) {
          mat = y === waterLevel ? (P.water ?? P.WATER ?? 14) : (P.waterDeep ?? P.WATER_DEEP ?? 15);
        }
        // Beach sand (Shoreline-specific, skip for desert)
        else if (!isDesert && x < -40 && surfaceY <= 2 && y <= surfaceY) {
          mat = y === surfaceY
            ? (P.sandLight ?? P.SAND_LIGHT ?? 1)
            : (y === surfaceY - 1
              ? (P.sandDark ?? P.SAND_DARK ?? 2)
              : (P.dirt ?? P.DIRT ?? 5));
        }
        // Surface
        else if (y === surfaceY) {
          if (isDesert) {
            mat = (x + z) % 3 === 0 ? (P.sandDark ?? P.SAND_DARK ?? 2) : (P.sandLight ?? P.SAND_LIGHT ?? 1);
          } else if (surfaceY <= 2) {
            mat = P.grass ?? P.GRASS ?? 3;
          } else if (surfaceY <= 5) {
            mat = (x + z) % 3 === 0 ? (P.grassDark ?? P.GRASS_DARK ?? 4) : (P.grass ?? P.GRASS ?? 3);
          } else {
            mat = (x + z) % 2 === 0 ? (P.stone ?? P.STONE ?? 6) : (P.grassDark ?? P.GRASS_DARK ?? 4);
          }
        }
        // Sub-surface
        else if (y >= surfaceY - 2) {
          mat = isDesert ? (P.sandDark ?? P.SAND_DARK ?? 2) : (P.dirt ?? P.DIRT ?? 5);
        } else {
          mat = P.stoneDark ?? P.STONE_DARK ?? 7;
        }
...
function generateRoads(bounds: MapdefBounds, P: Record<string, number>): void {
  ...
  for (let x = -60; x <= 80; x++) {
    for (let zOff = -2; zOff <= 2; zOff++) {
      const z = 0 + zOff;
      const surfaceY = heightAt(x, z, bounds);
      setVoxel(x, surfaceY, z, P.road ?? P.ROAD ?? 16);
...
function makeBuilding(spec: BuildingSpec, bounds: MapdefBounds, P: Record<string, number>): void {
  ...
  for (let story = 0; story < spec.stories; story++) {
    ...
    if (spec.hasWindows) {
      const windowColor = P.window ?? P.WINDOW ?? 17;
  ...
  if (spec.hasBalcony && spec.stories >= 2) {
    const woodColor = P.wood ?? P.WOOD ?? 10;
    const woodDark = P.woodDark ?? P.WOOD_DARK ?? 11;
...
function generateBuildings(bounds: MapdefBounds, P: Record<string, number>): void {
  ...
  makeBuilding({ x: -60, z: -16, w: 14, d: 8, h: 6, stories: 1, material: P.concrete ?? P.CONCRETE ?? 8, roofMaterial: P.metal ?? P.METAL ?? 18, hasDoors: true, hasWindows: false }, bounds, P);
  makeBuilding({ x: -46, z: 4, w: 10, d: 8, h: 5, stories: 1, material: P.concreteDark ?? P.CONCRETE_DARK ?? 9, roofMaterial: P.metal ?? P.METAL ?? 18, hasDoors: true, hasWindows: false }, bounds, P);

  const dockBaseY = heightAt(-65, 0, bounds);
  fillBox(-70, dockBaseY, -2, -62, dockBaseY, 2, P.stone ?? P.STONE ?? 6);
  fillBox(-80, dockBaseY - 1, -1, -70, dockBaseY, 1, P.stone ?? P.STONE ?? 6);

  makeBuilding({ x: -2, z: -20, w: 8, d: 10, h: 4, stories: 3, material: P.brick ?? P.BRICK ?? 12, roofMaterial: P.roofTile ?? P.ROOF_TILE ?? 13, hasDoors: true, hasWindows: true }, bounds, P);
  makeBuilding({ x: 10, z: 2, w: 10, d: 8, h: 5, stories: 2, material: P.concrete ?? P.CONCRETE ?? 8, roofMaterial: P.roofTile ?? P.ROOF_TILE ?? 13, hasDoors: true, hasWindows: true, hasBalcony: true }, bounds, P);
  makeBuilding({ x: -8, z: 12, w: 6, d: 6, h: 4, stories: 1, material: P.wood ?? P.WOOD ?? 10, roofMaterial: P.woodDark ?? P.WOOD_DARK ?? 11, hasDoors: true, hasWindows: true }, bounds, P);
  makeBuilding({ x: 22, z: -12, w: 7, d: 7, h: 4, stories: 2, material: P.brick ?? P.BRICK ?? 12, roofMaterial: P.roofTile ?? P.ROOF_TILE ?? 13, hasDoors: true, hasWindows: true }, bounds, P);
  makeBuilding({ x: -12, z: -6, w: 8, d: 6, h: 4, stories: 2, material: P.concreteDark ?? P.CONCRETE_DARK ?? 9, roofMaterial: P.metal ?? P.METAL ?? 18, hasDoors: true, hasWindows: true }, bounds, P);
}
...
function generateFort(bounds: MapdefBounds, P: Record<string, number>): void {
  ...
  const stoneColor = P.stone ?? P.STONE ?? 6;
  const stoneDark = P.stoneDark ?? P.STONE_DARK ?? 7;
  const grassColor = P.grass ?? P.GRASS ?? 3;
  const dirtColor = P.dirt ?? P.DIRT ?? 5;
  const woodColor = P.wood ?? P.WOOD ?? 10;
...
function generateCover(bounds: MapdefBounds, P: Record<string, number>): void {
  ...
  const sandDark = P.sandDark ?? P.SAND_DARK ?? 2;
  const woodDark = P.woodDark ?? P.WOOD_DARK ?? 11;
  const stoneColor = P.stone ?? P.STONE ?? 6;
...
function generateWater(bounds: MapdefBounds, P: Record<string, number>, waterLevel: number): void {
  ...
  const waterColor = P.water ?? P.WATER ?? 14;
  const waterDeep = P.waterDeep ?? P.WATER_DEEP ?? 15;
